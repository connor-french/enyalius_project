...
# job script.
#
# When using the PBS variables please ensure that there is one "#" before the 
# varible or it will not be interpreted correctly by the scheduler
#
...
#PBS -m abe
#PBS -M cfrench@amnh.org

### Optionally set the destination for your program's output
### Specify localhost and an NFS filesystem to prevent file copy errors.
### If no filenames are specified then the files are named <JOBNAME>.o<JOBID> and <JOBNAME>.e<JOBID>

#PBS -e /home/cfrench/ihe_class_errors.e
#PBS -o /home/cfrench/ihe_class_output.o
...
#MPIRUN=`which mpirun`
source ~/.bashrc
micromamba activate /home/cfrench/nas5/micromamba/envs/mlpy
##mpirun --mca btl ^tcp -machinefile $PBS_NODEFILE -np ${NPROCS} python3 model_selection.py

### Or, just run your serial program
python3 $HOME/model_classification.py

...
def main():
    ...
    out_prefix_hinge = "sims_2023-11-18_hinge"
    ...
    lin_sim_params = list(itertools.product(*[
       ...
       #ancestral_merge_size_list, 
        ...
        [0],
        sp_merge_time_list]))
    #hinge_sim_params = list(itertools.product(*[max_k, Nm, ancestral_n_multiplier, recomb_rate, thresh]))
    ...
    # sims got interrupted by a timeout, so continuing where I left off
    lin_sim_params_first = lin_sim_params_first[2887:]
    lin_sim_params_first = lin_sim_params_first[98:]

    # # ### linear sims
    num_param_combos = len(lin_sim_params_first)
    ...
    for sim in range(first_sim, last_sim):
        # print what's going on with simulations
        print(lin_sim_params_first[sim])
        run_sims(params = lin_sim_params_first[sim], 
                 mut_rates = mut_rates,
                 raster = r, 
                 admix_rast = admix_rast, 
                 #merge_time = 22000,

...
def main():
    ...
    raster_path = "projections_per.tif"
    admix_rast_path = "admix_id_rast_per.tif"
    ...
    out_prefix_linear = "sims_2024-03-21_per_linear"
    out_prefix_hinge = "sims_2024-03-21_per_hinge"
    ...
    ## a1 is the smaller population size for perditus
    ancestral_size_a1 = np.linspace(5e4, 1e6, n_p, dtype=int)

    ancestral_size_a2 = np.linspace(5e5, 3e6, n_p, dtype=int)

    # since the empirical pi for admix pop 2 (0.157641) is nearly equal to admix pop 3 (0.156661), I'm setting the ancestral size for a3 to be equal to a2
    ...
    mid_ind = len(hinge_sim_params) // 2
    hinge_sim_params_first = [hinge_sim_params[i] for i in range(mid_ind + 1)]
    hinge_sim_params_second = [hinge_sim_params[i] for i in range(mid_ind, len(hinge_sim_params))]

    # ### hingeear sims
    num_param_combos = len(hinge_sim_params_second)
    ...
    for sim in range(first_sim, last_sim):
        run_sims(params = hinge_sim_params_second[sim], 
                 mut_rates = mut_rates,
                 raster = r, 
                 admix_rast = admix_rast, 
                 #merge_time = 22000,

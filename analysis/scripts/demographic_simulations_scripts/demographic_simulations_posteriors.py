...
def run_sims(
    ...
    for sim in range(n_sims):
        ...
        try:
            ...
            coal_1d = np.array(coal_list)[:-4]

            coal_array = np.reshape(coal_1d, newshape = raster[0].shape)
...
if __name__=="__main__":
    ...
    raster_path = "/Users/connorfrench/Dropbox/Old_Mac/School_Stuff/CUNY/enyalius-phylogeography/enyalius/analysis/output/sdm_projections/projections_ihe.tif"
    admix_rast_path = "/Users/connorfrench/Dropbox/Old_Mac/School_Stuff/CUNY/enyalius-phylogeography/enyalius/analysis/output/admix_rasters/admix_id_rast_ihe.tif"
    ...
    localities_path = "/Users/connorfrench/Dropbox/Old_Mac/School_Stuff/CUNY/enyalius-phylogeography/enyalius/analysis/data/enyalius_locs_genetics.csv"
    localities = pd.read_csv(localities_path)

    localities = localities[localities["species"] == "iheringii"]

    out_prefix = "sims_2024-03-22_posterior"
    ...
    max_k = np.round(np.random.normal(loc=1587, scale=((2893-191)/4), size=n_p), 0)

    recomb_rate = [1e-9]
    
    mut_rates = [1e-9] 
    
    Nm = np.random.normal(loc=1.4656758, scale=((2.59292006-0.22049499)/4), size=n_p)
    ...
    ancestral_size_a1 = np.round(np.random.normal(loc=2552127, scale=((3499479-1853824)/4), size=n_p), 0)

    ancestral_size_a2 = np.round(np.random.normal(loc=326013, scale=((415122-241116)/4), size=n_p), 0)

    ancestral_size_list = [[ancestral_size_a1[i], ancestral_size_a2[i]] for i in range(len(ancestral_size_a1))]

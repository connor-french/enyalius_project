...
locs <- read_csv(here("assembly", "processed_localities", "locs.csv"))

# cluster thresholds 
## update with snakemake input
...
share_paths <- list.files(here("assembly", "full", "sharing_matrices"), full.names = TRUE)
...
pca_batch_paths <- list.files(here("assembly", "full", "pca_batch"), pattern = ".csv", full.names = TRUE)
...
## convert to snakemake input
locs_ingroup <- locs %>% 
         filter(species %in% c("catenatus", "iheringii", "perditus", "pictus"))

# read in pc variances
## convert to snakemake
pca_var_paths <- list.files(here("assembly", "full", "pca_clust"), pattern = "var", full.names = TRUE)

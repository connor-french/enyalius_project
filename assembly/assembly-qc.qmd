...
locs <- snakemake@input[["locs"]]

# cluster thresholds 
clust_thresh <- paste0("clust", snakemake@config[["CLUSTTHRESHFULL"]])
...
ip_stats_94 <- read_ipyrad_stats(snakemake@input[["ip_stats_94"]])
...
share_paths <- snakemake@input[["share_paths"]]
...
pca_batch_paths <- snakemake@input[["pca_batch_paths"]]
...
ip_stats_files <- snakemake@config[["IPSTATS"]]
...
outgroups <- snakemake@config[["OUTSHORT"]]
...
locs_ingroup <- locs %>% 
         filter(species %in% snakemake@config[["INLONG"]])

# read in pc variances
pca_var_paths <- snakemake@input[["clust_var_paths"]]
...
pca_clust_paths <- snakemake@input[["clust_pc_paths"]]
